...
def apicurso(request):
    ...
    if int(grafico) == 0 or grafico == None:
        cur.execute(
            'select graph_curso.nome as nome, graph_curso.id as curso_id from graph_curso left join graph_cursocampus on graph_curso.id = graph_cursocampus.curso_id left join  graph_campus on graph_cursocampus.campus_id = graph_campus.id where graph_campus.id = {} and graph_curso.nome != \'Não Aplica\' order by nome'.format(
                campus))
        for i in cur.fetchall():
    ...
    if int(grafico) > 1:
        cur.execute(
            'select distinct curso, curso_id from graph_view{} where campus_id = {} order by curso;'.format(grafico,
                                                                                                            campus))
        for i in cur.fetchall():
            cursos.append({'id': i[1], 'nome': i[0]})
